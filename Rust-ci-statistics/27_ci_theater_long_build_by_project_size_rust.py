...
def process_cohort(builds_file: str, sizes_file: str, cohort_name: str):
    ...
    q3 = df["Avg Duration (min)"].quantile(0.75)
    ...
    return df, category_order, median, q3, p95
...
def composite_plot(mono_args, poly_args, output_file: str, plot_style: str):
    ...
    df_mono, cat_order_mono, median_mono, q3_mono, p95_mono = mono_data
    df_poly, cat_order_poly, median_poly, q3_poly, p95_poly = poly_data

    # Print summary info for Monoglot
    print(f"\n--- Build Duration Analysis for Monoglot Cohort ---")
    print(f"Median: {median_mono:.2f} min")
    ...
    print(f"Median: {median_poly:.2f} min")
    ...
    # Annotate each category with median, Q1 and Q3 above the box in the Monoglot subplot
    group_stats = df_mono.groupby('Category')["Avg Duration (min)"].agg(
        median='median', 
        first_quartile=lambda x: x.quantile(0.25), 
        third_quartile=lambda x: x.quantile(0.75)
    )
    ...
    # Annotate each category with median, Q1 and Q3 above the box in the Polyglot subplot
    group_stats = df_poly.groupby('Category')["Avg Duration (min)"].agg(
        median='median', 
        first_quartile=lambda x: x.quantile(0.25), 
        third_quartile=lambda x: x.quantile(0.75)
    )

    #plt.suptitle("Comparison of Build Durations: Monoglot vs Polyglot Projects", fontsize=16)
