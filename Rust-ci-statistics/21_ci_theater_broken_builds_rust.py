...
def fetch_runs(owner, name, max_runs=5000):
    all_runs = []
    page = 1
    per_page = 100

    while len(all_runs) < max_runs:
        url = f"https://api.github.com/repos/{owner}/{name}/actions/runs?per_page={per_page}&page={page}"
        try:
...
for p in projects:
    ...
    runs = fetch_runs(p["owner"], p["name"], max_runs=500)
    if not runs:
        results.append({
            "name": p["name"],
            "Runs Analyzed": 0,
...
# Save to CSV
with open("data/21_ci_theater_broken_builds_rust.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=results[0].keys())
    writer.writeheader()
    writer.writerows(results)

print("\n✅ Results saved to 21_ci_theater_broken_builds_rust.csv")
