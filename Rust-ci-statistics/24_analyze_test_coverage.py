#!/usr/bin/env python3
"""
Generate CI Theater summary plots for Rust projects from test coverage data.
...
def get_args():
    ...
    parser = argparse.ArgumentParser(description="Generate CI Theater summary plots for Rust projects.")
    parser.add_argument(
        "--input-csv",
        default="data/24_test_coverage.csv",
        help="Path to the input CSV file from 24_collect_test_coverage.py.",
    )
    parser.add_argument(
        "--output-dir", default="figures", help="Directory to save the output plots and CSVs."
    )
...
def main():
    """Main function to generate plots and summaries."""
    args = get_args()
    ...
    summary_csv_path = os.path.join(args.output_dir, "ci_summary_table.csv")
    ...
    # ----------------------------- Plots ------------------------------
    bundle_pdf_path = os.path.join(args.output_dir, "ci_plots_bundle.pdf")
    with PdfPages(bundle_pdf_path) as pdf:
        ...
        save_fig(fig1, os.path.join(args.output_dir, "ci_adoption_breakdown.png"), pdf)
        ...
        save_fig(fig2, os.path.join(args.output_dir, "ci_funnel_counts.png"), pdf)
        ...
        if len(coverage_values) > 0:
            ...
            ax3.set_ylim(0, 100)
            save_fig(fig3, os.path.join(args.output_dir, "coverage_boxplot.png"), pdf)
            ...
            ax4.set_ylim(0, 100)
            save_fig(fig4, os.path.join(args.output_dir, "coverage_violin.png"), pdf)
        else:
            ...
            ax_note.text(0.5, 0.5,
                         ...
                         ha="center", va="center")
            save_fig(fig_note, os.path.join(args.output_dir, "coverage_no_data.png"), pdf)

    print(f"[OK] Wrote summary table: {summary_csv_path}")
    print(f"[OK] Wrote figures to: {args.output_dir}/")
